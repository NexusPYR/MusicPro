...
class MainActivity : BaseActivity() {
    ...
    private val selectNcmLauncher = registerForActivityResult(
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        btnShowFavorites.setOnClickListener {
            ...
            updateLibraryView()
            btnShowFavorites.setImageResource(if (isShowingFavorites) R.drawable.ic_favorite else R.drawable.ic_favorite_border)
        }
    ...
    private fun syncMiniPlayerUI(controller: MediaController) {
        ...
        if (song?.cover != null) {
            ...
            ivMiniCover.setImageResource(R.drawable.ic_play)
    ...
    private fun showSettingsDialog() {
        ...
        val btnConvert = view.findViewById<Button>(R.id.btn_convert_ncm)
        val prefs = getSharedPreferences("music_pro", Context.MODE_PRIVATE)
        
        btnConvert.setOnClickListener {
            selectNcmLauncher.launch(arrayOf("*/*"))
        }
        ...
        Thread {
            contentResolver.query(collection, projection, selection, null, "${MediaStore.Audio.Media.DATE_ADDED} DESC")?.use { cursor ->
                ...
                while (cursor.moveToNext()) {
                    ...
                    // 屏蔽特定文件夹：录音文件夹
                    if (path.contains("/Music/Recordings/", ignoreCase = true)) {
                        continue
                    }
    ...
    private fun traverseDirectory(dir: DocumentFile, songs: MutableList<Song>) {
        // 屏蔽录音文件夹
        if (dir.name?.equals("Recordings", ignoreCase = true) == true) {
            return
        }
    ...
    private fun showSongDetails(song: Song) {
        ...
        try {
            val projection = arrayOf(MediaStore.Audio.Media.SIZE, MediaStore.Audio.Media.DATA)
            contentResolver.query(song.uri, projection, null, null, null)?.use { cursor ->
                if (cursor.moveToFirst()) {
                    ...
                    if (dataIndex != -1) {
                        pathStr = cursor.getString(dataIndex) ?: "未知"
                    }
                }
            }
        } catch (e: Exception) {
    ...
    private fun copyToClipboard(text: String) {
        ...
        val clip = ClipData.newPlainText("文件路径", text)
    ...
    private fun formatFileSize(size: Long): String {
        ...
        return String.format(java.util.Locale.getDefault(), "%.2f %s", size / Math.pow(1024.0, digitGroups.toDouble()), units[digitGroups])
    }
