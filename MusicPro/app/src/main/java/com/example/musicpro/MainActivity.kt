...
class MainActivity : AppCompatActivity() {
    ...
    private lateinit var searchView: SearchView // 🌟 新增：搜索框

    private var isShowingFavorites = false
    private var masterSongList = mutableListOf<Song>()
    private var currentSearchQuery = "" // 🌟 新增：保存当前搜索关键词
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        searchView = findViewById(R.id.search_view) // 🌟 绑定搜索框
        ...
        libraryAdapter = SongAdapter(
            ...
            onItemClick = { song -> playFromLibrary(song) },
    ...
    private fun setupController() {
        mediaController?.addListener(object : Player.Listener {
            ...
            override fun onMediaItemTransition(item: MediaItem?, reason: Int) {
                val song = masterSongList.find { it.id.toString() == item?.mediaId }
                tvMiniTitle.text = song?.title ?: item?.mediaMetadata?.title ?: "就绪"
    ...
    private fun updateLibraryView() {
        var filteredList = masterSongList.toList() // Start with a copy of the master list
    ...
    private fun showSettingsDialog() {
        ...
        switchPause.setOnCheckedChangeListener { _, isChecked -> with(prefs.edit()) { putBoolean("pause_on_unplug", isChecked); apply() } }
    ...
    private fun showPlaybackQueueDialog() {
        ...
        val dialogAdapter = SongAdapter(queueSongs, { s ->
            ...
            // No long click action in playlist
        })
